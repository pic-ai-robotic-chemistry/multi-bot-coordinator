...
def plot_bottle_db(fig, ax, offset, makespan_offset, database_file_name):
    ...
    if "STC202407060956465669_1" in bottle_table.keys():
        order = ["STC202407060956465669_1", "STC202407060956465669_3", "STC202407060956465669_2"]
        sorted_bottle_table = {k: bottle_table[k] for k in order}
        bottle_table = sorted_bottle_table

    # For plotting: If it’s a mixed experiment, sort the bottles
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Draw Scheduled table")
    parser.add_argument("--num_robot", type=int, default=2, help="num of robot")
    ...
    # experiment 1
    bottle_table, ws_table, makespan = plot_bottle_db(fig, ax, cumul_offset, makespan_offset, database_file_1)
    cumul_offset += len(bottle_table)
    makespan_offset += makespan
    specific_time_1 = makespan_offset
    ylabels = [
        ("{}-{}".format(bottle_table[b_id][0]["expr_name"][0:2], b_id.split("_")[1]))
    ...
    # experiment 2
    bottle_table, ws_table, makespan = plot_bottle_db(fig, ax, cumul_offset, makespan_offset, database_file_2)
    cumul_offset += len(bottle_table)
    makespan_offset += makespan
    specific_time_2 = makespan_offset
    ylabels = [
        ("{}-{}".format(bottle_table[b_id][0]["expr_name"][0:2], b_id.split("_")[1]))
        for b_id in bottle_table.keys()
    ]
    ...
    # experiment 3
    bottle_table, ws_table, makespan = plot_bottle_db(fig, ax, cumul_offset, makespan_offset, database_file_3)
    cumul_offset += len(bottle_table)
    makespan_offset += makespan
    specific_time_3 = makespan_offset
    ylabels = [
        ("{}-{}".format(bottle_table[b_id][0]["expr_name"][0:2], b_id.split("_")[1]))
        for b_id in bottle_table.keys()
    ]
    ...
    # experiment 4
    bottle_table, ws_table, makespan = plot_bottle_db(fig, ax, cumul_offset, makespan_offset, database_file_4)
    cumul_offset += len(bottle_table)
    makespan_offset += makespan
    specific_time_4 = makespan_offset
    ylabels = [
        ("{}-{}".format(bottle_table[b_id][0]["expr_name"][0:2], b_id.split("_")[1]))
        for b_id in bottle_table.keys()
    ]
    ...
    ax.axvline(specific_time_4, color="r", linestyle="--")

    ax.set_xlim([start_time, specific_time_4])
    # ax.set_ylabel("squential experiments", fontsize=16)

    ax.set_xticks([start_time, specific_time_1, specific_time_2, specific_time_3, specific_time_4])
    ax.set_xticklabels(
        [start_time, specific_time_1, specific_time_2, specific_time_3, specific_time_4], fontsize=16
    )
    ...
    ax1.set_xlim([start_time, specific_time_4])
    ax1.set_xticks([start_time, specific_time_0])
    ax1.set_xticklabels([start_time, specific_time_0], fontsize=16)

    # ax1.set_xlabel("Time(min)", fontsize=16)
    # ax1.set_ylabel("parallel experiments", fontsize=16)
    plt.tight_layout()
    plt.show()
