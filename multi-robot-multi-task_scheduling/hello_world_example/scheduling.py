...
class TaskScheduling:
    def __init__(
        self, flask_route="/scheduling_test", flask_port=5051, debug_mode=False
    ):
        # --------------------LOG-----------------------------
        ...
        self.cur_tasks_for_robots = {
            ...
            "ws_arr": collections.defaultdict(
                list
            ),  # The station each robot is assigned to
        }
        ...
        def scheduling():
            ...
            with self.msg_lock:
                ...
                self.flag_reveive_first_taskflow, self.ws_in_highflux = update_db(
                    self.flag_reveive_first_taskflow,
                    self.ws_ptr,
                    full_data,
                    flask_conn_sql,
                )
    ...
    def run_flask(self):
        ...
        self.app.run(
            host="0.0.0.0", port=self.flask_port, debug=True, use_reloader=False
        )
        ...
        while not self.exit_event.is_set():
            ...
            with self.msg_lock:
                ...
                if not self.flag_gen_table and self.flag_receive_new_taskflow:
                    ...
                    self.bottle_location = find_bottle_location_from_db(
                        main_loop_conn_sql
                    )
    ...
    def handle_ws_oper(self, conn_sql):
        ...
        while bool(self.ws_ptr_matched_tasks["do_start"]) and self.flag_gen_table:
            ...
            for ws_code, bottles in dict(self.ws_ptr_matched_tasks["do_start"]).items():
                ...
                if start:
                    ...
                    with self.msg_lock:
                        self.send_next_assignment(
                            "fake_robot" + str(time.time()), ws_code, "start", conn_sql
                        )
        ...
        for oper in ["dripping", "test", "recycle"]:
            while bool(self.ws_ptr_matched_tasks["do_" + oper]):
                ...
                for ws_code, bottles in dict(
                    self.ws_ptr_matched_tasks["do_" + oper]
                ).items():
                    if "electrocatalysis" in ws_code:
                        start = all(
                            self.bottle_location[b["b_id"]]
                    ...
                    if start:
                        ...
                        with self.msg_lock:
                            self.send_next_assignment(
                                "robot_platform", ws_code, cmd, conn_sql
                            )
    ...
    def send_assignment(self, oper: str, conn_sql: sqlite3.Connection):
        ...
        flag_ws_conflict, ws_code_conflict_0, ws_code_conflict_1 = (
        ...
        while flag_ws_conflict and self.flag_gen_table:
            ...
            if not send_robot_0:
                ...
                with self.msg_lock:
                    r_status, r_battery = find_robot_status(
                        self.robot_list[0], conn_sql
                    )
                ...
                while not send_msg_success:
                    ...
                    with self.msg_lock:
                        ...
                        print(
                            Fore.GREEN
                            + Style.BRIGHT
                            + f"Resend msg:{repeat_msg_num} times"
                            + Style.RESET_ALL
                        )
                    self.wait_assign_queue_empty()
                    with self.msg_lock:
                        send_msg_success = check_bottle_records(
                            one_assign_tmp, conn_sql
                        )
                if send_msg_success:
                    print(Fore.GREEN + Style.BRIGHT + "operating..." + Style.RESET_ALL)

            # Check if the tasks at the conflicting workstation are completed
            if self.check_if_robot_tasks_done(
                oper, self.robot_list[0], ws_code_conflict_0
            ):
                # Remove the conflicting workstation
                self.cur_tasks_for_robots["ws_arr"][self.robot_list[0]].remove(
                    ws_code_conflict_0
                )
        ...
        while flag_ws_conflict and self.flag_gen_table:
            ...
            if (
                self.check_if_robot_tasks_done(oper, "robot_platform", ws_id_platform)
                == True
            ):
                self.cur_tasks_for_robots["ws_arr"]["robot_platform"].remove(
                    ws_id_platform
                )
        ...
        robot_ptr = {
            robot_name: len(ws_id_arr)
            - 1  # Here, subtracting one is for convenient indexing
        ...
        for robot_name, ws_id_arr in self.cur_tasks_for_robots["ws_arr"].items():
            for ws_code in ws_id_arr:
                all_tasks_done &= self.check_if_robot_tasks_done(
                    oper, robot_name, ws_code
                )

        if all_tasks_done == True:
            all_tasks_done &= self.check_if_matched_tasks_done(
                self.ws_ptr_matched_tasks, oper
            )
        ...
        while not all_tasks_done and self.flag_gen_table:
            ...
            for robot_name, ws_id_arr in self.cur_tasks_for_robots["ws_arr"].items():
                if robot_ptr[robot_name] >= 0 and robot_can_proc_flag[robot_name]:
                    ...
                    with self.msg_lock:
                        one_assign_tmp = self.send_next_assignment(
                            robot_name, ws_code, oper, conn_sql
                        )
                    ...
                    if send_msg_success:
                        print(
                            Fore.GREEN + Style.BRIGHT + "operating..." + Style.RESET_ALL
                        )
            ...
            # If a new task is inserted, the scheduling table is regenerated.
            if self.flag_receive_new_taskflow == True:
                self.flag_gen_table = False
                return
    ...
    def gen_assignment(self, oper: str, conn_sql: sqlite3.Connection):
        ...
        with self.msg_lock:
            ...
            self.cur_tasks_for_robots["assign"] = self.assign_tasks(
                do_oper_tasks, oper, conn_sql
            )
    ...
    def assign_tasks(self, do_oper_tasks, oper, conn_sql: sqlite3.Connection):
        groups = {
            r_name: defaultdict(list)
            for r_name in (self.robot_list + self.robot_platform_list)
        }
        ...
        if oper == "put":
            ...
            sorted_take_tasks = sorted(
                do_oper_tasks.items(), key=lambda x: len(x[1]), reverse=True
            )
            for ws_code, tasks in sorted_take_tasks:
                if ws_code in self.ws_in_highflux:
                    ...
                    exchange_station_tasks = [
                        task
                        for task in tasks
                        if task.get("next_step_ws_code_fjspb")
                    ...
                    other_tasks = [
                        ...
                        for task in tasks
                        if task.get("next_step_ws_code_fjspb")
                        not in ["multi_robots_exchange_workstation"]
                    ]
                    if exchange_station_tasks:
                        groups[self.robot_list[0]][ws_code].extend(
                            exchange_station_tasks
                        )
                        if other_tasks:
    ...
    def send_next_assignment(
        ...
        if oper == "finish":
            ...
            for b_task in self.cur_tasks_for_robots["assign"][robot_name][ws_code]:
                task_info = fetch_task_info_from_db(
                    b_task["b_id"], int(b_task["fjspb_index"]), conn_sql
                )
                b_id_actual = find_b_no_map_from_db(
                    b_task["b_id"], actual_to_vials=False, conn_sql=conn_sql
                )
                ...
                b_msg = {
                    ...
                    "bottleCode": (
                        b_id_actual if b_id_actual else b_task["b_id"].split("_")[1]
                    ),
                    "material": None,
                    "material_code": None,
                    "value": None,
                ...
                if isinstance(task_info["detail"]["material"], Iterable):
                    for idx, values in enumerate(task_info["detail"]["material"]):
                        ...
                        if material_full_info.split("_")[1] != "None":
                            b_msg["concentration"] = float(
                                material_full_info.split("_")[1]
                            )
                        if material_full_info.split("_")[2] != "None":
                            b_msg["concentration_unit_code"] = material_full_info.split(
                                "_"
                            )[2]
                        b_msg["material_code"] = task_info["detail"]["material_code"][
                            idx
                        ]
    ...
    def check_if_robot_tasks_done(self, oper, robot_name, ws_code) -> bool:
        return all(
            r_task[oper]
            for r_task in self.cur_tasks_for_robots["assign"][robot_name][ws_code]
        )
    ...
    def check_if_matched_tasks_done(self, tasks: dict, oper: str) -> bool:
        for ws_tasks in tasks.get("do_" + oper, {}).values():
            for task in ws_tasks:
                b_id = task["b_id"]
                if oper == "take":
                    if self.bottle_location[b_id] not in (
                        self.robot_list + self.robot_platform_list
                    ):
                        return False
                elif oper == "put":
                    if self.bottle_location[b_id] in (
                        self.robot_list + self.robot_platform_list
                    ):
                        return False
                if not task[oper]:
    ...
    def find_next_tasks(self, conn_sql: sqlite3.Connection):
        while self.ws_ptr <= self.makespan:
            ...
            if any(
                ws_ptr_matched_tasks.get(key)
                for key in ws_ptr_matched_tasks
                if key != "left"
            ):
                break
        ...
        for r_name, r_tasks in self.cur_tasks_for_robots["assign"].items():
            for ws_id, ws_tasks in r_tasks.items():
                self.cur_tasks_for_robots["assign"][r_name][ws_id] = [
                    fetch_task_info_from_db(
                        task["b_id"], task["fjspb_index"], conn_sql, full_info=True
                    )
                    for task in ws_tasks
                ]
        conn_sql.commit()
...
if __name__ == "__main__":
    ...
    try:
        ...
        TS = TaskScheduling(
            flask_route=flask_route, flask_port=flask_port, debug_mode=debug_mode
        )
        TS.run()
